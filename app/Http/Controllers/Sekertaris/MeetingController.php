...
class MeetingController extends Controller
{
    ...
    // {
    //     $meetings = Meeting::with('leader', 'secretary', 'minutes', 'rooms')->get();

    //     return view('sekertaris.meeting', compact('meetings'));

    // }
    public function index(Request $request)
    {
        if ($request->ajax()) {
            // $meetings = Meeting::with(['leader', 'secretary', 'minutes', 'rooms'])->select('meetings.*');
        ...
        ->select([
            'meetings.*',
            DB::raw("CONCAT(TO_CHAR(start_time, 'DD-MM-YYYY HH24:MI'), ' - ', TO_CHAR(end_time, 'HH24:MI')) as time_range")
        ]);
            return DataTables::of($meetings)
                ...
                //     return Carbon::parse($mt->start_time)->locale('id')->translatedFormat('H:i');
                // })
                // ->addColumn('end_time', function ($mt) {
                //     return Carbon::parse($mt->end_time)->locale('id')->translatedFormat('H:i');
                // })
                // ->editColumn('time_range', function ($mt) {
                //     return Carbon::parse($mt->start_time)->format('d-m-Y H:i') . ' - ' . Carbon::parse($mt->end_time)->format('H:i');
                // })
                ...
                ->filterColumn('time_range', function($query, $keyword) {
                    $query->whereRaw("CONCAT(TO_CHAR(start_time, 'DD-MM-YYYY HH24:MI'), ' - ', TO_CHAR(end_time, 'HH24:MI')) like ?", ["%{$keyword}%"]);
                })
    ...
    public function schedule(Request $request)
    {
        if ($request->ajax()) {
            ...
            // ->select([
            //     'meetings.*',
            //     DB::raw("CONCAT(DATE_FORMAT(meetings.start_time, '%H:%i'), ' - ', DATE_FORMAT(meetings.end_time, '%H:%i')) as time_range")
            // ]);
            return DataTables::of($meetings)
                ->addColumn('action', function ($mt) {
                        return '
                ...
                // ->filterColumn('time_range', function($query, $keyword) {
                //     $sql = "CONCAT(DATE_FORMAT(meetings.start_time, '%H:%i'), ' - ', DATE_FORMAT(meetings.end_time, '%H:%i')) like ?";
    ...
    public function meetingresult(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($meetings)
                ...
                // ->addColumn('summary_result', function ($mt) {
                //     return $mt->summaries->isNotEmpty() ? $mt->summaries->pluck('summary_result')->implode(', ') : 'No Summary';
                // })
                ->editColumn('status', function ($mt) {
                    return '<span class="badge badge-primary">' . $mt->status . '</span>';
                })
